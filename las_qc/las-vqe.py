...
parser.add_argument(
    ...
    type=float,
    default=1.35296239,
    help="distance of H2s from one another or C=C bond distance scaling in butadiene",
)
parser.add_argument(
...
print(
    "Ncore: ", ncore, "Ncas: ", ncas, "Ncas_sub: ", ncas_sub, "Nelec_cas: ", nelec_cas
)
...
def custom_excitations(
    num_spin_orbitals: int, num_particles: Tuple[int, int], num_sub: List[int]
...
if args.dist == 0.0:
    ...
    circ_for_counts = transpile(
        new_circuit, basis_gates=target_basis, optimization_level=0
    )
...
if args.dist == 0.0:
    ...
    vqe_ops = 0
    circ_list = transpile(
        algorithm.construct_circuit(params, hamiltonian),
...
if args.dist == 0.0:
    np.save(
        "results_{}_{}_vqe.npy".format(args.shots, args.dist),
        {
            "init_op_dict": init_op_dict,
            "init_ops": init_ops,
            "vqe_op_dict": vqe_op_dict,
            "vqe_ops": vqe_ops,
            "vqe_result": vqe_result,
            "vqe_en_vals": values,
            "vqe_counts": counts,
            "nuc_rep": las.energy_nuc(),
        },
    )
else:
    np.save(
