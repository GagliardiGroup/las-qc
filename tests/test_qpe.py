...
def test_hf():
    """Test the PySCF RHF"""
    xyz = get_geom("close")
    mol = gto.M(atom=xyz, basis="sto-3g", output="h4_sto3g.log", symmetry=False)
...
def test_frag_np_eig():
    ...
    for frag in range(len(las.ncas_sub)):
        hamiltonian = get_hamiltonian(
            frag, las.nelecas_sub, las.ncas_sub, h1_frag, h2_frag
        )

        # Set the backend
        backend = AerSimulator()
        qc = transpile(hamiltonian, backend)
        _qobj = assemble(qc, shots=1024)
